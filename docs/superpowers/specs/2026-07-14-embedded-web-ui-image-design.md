...
docker build --platform linux/amd64 -t ghcr.io/aurosk-star/starpay:v0.0.1-beta .
```
...
After verification, replace the existing beta image at `ghcr.io/aurosk-star/starpay:v0.0.1-beta` and verify the GHCR manifest digest. This mutable replacement is acceptable only because the requested tag is explicitly a beta tag.
